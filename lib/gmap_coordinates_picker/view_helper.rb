  ...
  module ViewHelper #:nodoc
    def render_gmap_coordinate_picker(options={})
      unless options[:static].to_s == 'true'
        ...
        lat_column_value = options[:object].present? ? options[:object].send(lat_column) : options[:default_coordinates][0]
        lng_column_value = options[:object].present? ? options[:object].send(lng_column) : options[:default_coordinates][1]
