class Gig < ActiveRecord::Base
  ...
  def post_immediately?
    self.schedule_post.post_immediately? && self.starts_at > Date.today
  end

  def post_a_week_before?
    self.schedule_post.post_a_week_before? && self.starts_at > Date.today
  end

  def post_a_day_before?
    self.schedule_post.post_a_day_before? && self.starts_at > Date.today
  end

  def post_the_day_off?
    self.schedule_post.post_the_day_off? && self.starts_at > Date.today
  end
    ...
    self.find_in_batches(batch_size: 1000) do |group|
      group.each do |gig|
        ...
        if user.present?
          ...
          if gig.post_immediately? || gig.post_a_week_before? || gig.post_a_day_before? || gg.post_the_day_off?
